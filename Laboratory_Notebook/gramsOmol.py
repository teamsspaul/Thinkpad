...
def ChemList(ChemicalFormula):
    ...
    4. If there are more than 99 of a single atom in your chemical
...
while (i <len(ChemicalFormula)-1):
    ...
    if re.search('[A-Z]',ChemicalFormula[i]): 
        if re.search('[a-z]',ChemicalFormula[i+1]):
            if re.search('_',ChemicalFormula[i+2]):
                if re.search('[0-9]',ChemicalFormula[i+4]):
                ...
                i=i+2
        elif re.search('_',ChemicalFormula[i+1]):
            if re.search('[0-9]',ChemicalFormula[i+3]):
                print(ChemicalFormula[i:i+4])
                i=i+4
            else:
