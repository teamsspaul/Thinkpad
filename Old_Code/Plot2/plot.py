...
a,index=var.File_To_Plotable_Matrix(var.BenchmarkInfoDir,var.filename)
...
if var.Compare_Data:
    a_2,index_2=var.File_To_Plotable_Matrix(var.BenchmarkInfoDir,var.filename_2)
...
if var.Center_Title and var.No_Title_Mod:
    fig.suptitle("\n".join(wrap(var.Title,60)),fontsize=var.TitleFontSize,fontweight=var.TitleFontWeight,fontdict=var.font,ha='center')
if var.Center_Title and not var.No_Title_Mod:
    fig.suptitle("\n".join(wrap(var.Title,60)),fontsize=var.TitleFontSize,
                 fontweight=var.TitleFontWeight,fontdict=var.font,ha='center',y=var.TitleYPosition)
if not var.Center_Title and not var.No_Title_Mod:
    fig.suptitle("\n".join(wrap(var.Title,60)),fontsize=var.TitleFontSize,
                 fontweight=var.TitleFontWeight,fontdict=var.font,ha='center',
...
########################################################################################
    ...
    for k in ranges:                              ############################
        ...
        if var.XValues != 1000 and var.PlotType == "XY" and var.Second_Y_Values:
            ax[j].plot(a[start:end+1,var.XValues],a[start:end+1,var.YValues2],
                       ...
                       label=unique_names[check])
            #check=check+1
        elif var.PlotType == "XY" and var.Second_Y_Values:
        ...
        #Plot each set of points, if XValues == 1000, plot index numbers
        if var.XValues != 1000 and var.PlotType == "XY":
                ...
                if var.Compare_B:
                    ...
                    if var.NumberOfSTDs[0]==1:
                        ax[j].plot(XSpace,np.ones(len(XSpace))*0.32,color=var.Compare_Bar,
                               linestyle=var.C_BarLine,alpha=var.C_BarAlpha)
...
if var.legend and var.Center_On_Graph and var.SS==1:
    ...
    ax[0].legend(handles,labels,loc='center left', bbox_to_anchor=(var.BBOXX,var.BBOXY),
                 fontsize=var.LegendFontSize,prop=var.font,ncol=var.NumberOfLegendColumns)
...
if var.legend and not var.Center_On_Graph and len(var.PlotSetup) > 1 and var.SS==1:
    ax[0].legend(handles,labels,loc='center right',fontsize=var.LegendFontSize,prop=var.font,ncol=var.NumberOfLegendColumns)
...
try:
    if not doiplot:
        ...
        if(Nplots==1 and Compare==False and STDcases>1):
            ...
            #From pre loop
            a_2,index_2=var.File_To_Plotable_Matrix(var.BenchmarkInfoDir,var.filename_2)
            (a_2,unique_names_2,ranges_2)=var.Group_by_Parameter(a_2,var.Group_by)
            #from inside loop
            a=a_2
            ranges=ranges_2
            index=index_2
